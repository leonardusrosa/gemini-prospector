...
KNOWN_AGENTS = ("generic", "antigravity", "codex", "claude-code", "opencode", "hermes")
...
def static_capabilities() -> dict:
    ...
    return {
        ...
        "prospectorCrmMcp": {
            "available": crm.exists(),
            "path": str(crm),
            "transport": "stdio",
        },
        "skills": {
            "available": skills.is_dir(),
            "path": str(skills),
        },
        "openDesign": {
        ...
        "imageGeneration": {
            "runtimeProbeRequired": True,
            "note": "Agent-specific capability. Canonical hero templates are the fallback.",
        },
        "github": {
            "runtimeProbeRequired": True,
        },
...
def write_runtime(agent: str, workspace: Path) -> tuple[Path, Path]:
    ...
    runtime = {
        ...
        "adapter": str(CORE / "adapters" / f"{agent}.md")
        if (CORE / "adapters" / f"{agent}.md").exists()
        else str(CORE / "adapters" / "generic.md"),
...
def print_human_doctor(agent: str, caps: dict) -> None:
    ...
    for label, ok, detail in rows:
        ...
        print(f"{label:30} {state:8} {detail or ''}")
    print()
    print("Runtime-only capabilities must be probed by the active agent: MCP connectivity, browser,")
    print("image generation, GitHub/Vercel credentials, and OpenDesign daemon reachability.")
    print(f"Core status: {'PASS' if core_ok(caps) else 'FAIL'}")


def cmd_doctor(args: argparse.Namespace) -> int:
    caps = static_capabilities()
    payload = {"agent": args.agent, "corePass": core_ok(caps), "capabilities": caps}
    if args.json:
        print(json.dumps(payload, ensure_ascii=False, indent=2))
    else:
        print_human_doctor(args.agent, caps)
    return 0 if payload["corePass"] else 1


def cmd_setup(args: argparse.Namespace) -> int:
    workspace = Path(args.workspace).expanduser().resolve() if args.workspace else ROOT
    runtime_path, mcp_path = write_runtime(args.agent, workspace)
    print(f"Generated runtime descriptor: {runtime_path}")
    print(f"Generated portable MCP config: {mcp_path}")
    print("Next: make your agent read AGENTS.md, import the generated MCP config using its native")
    print("configuration mechanism, probe capabilities, and use the generic adapter if no dedicated adapter exists.")
    return 0
...
def cmd_instructions(args: argparse.Namespace) -> int:
    adapter = CORE / "adapters" / f"{args.agent}.md"
    if not adapter.exists():
        adapter = CORE / "adapters" / "generic.md"
...
def cmd_self_test(_: argparse.Namespace) -> int:
    ...
    if not Path(crm_args[0]).exists():
        print(json.dumps({"pass": False, "missingCrmTarget": crm_args[0]}, indent=2))
        return 1
    print(json.dumps({"pass": True, "requiredFiles": len(required)}, indent=2))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    doctor = sub.add_parser("doctor", help="Inspect local static capabilities without claiming runtime connectivity.")
    doctor.add_argument("--agent", choices=KNOWN_AGENTS, default="generic")
    doctor.add_argument("--json", action="store_true")
    doctor.set_defaults(func=cmd_doctor)

    setup = sub.add_parser("setup", help="Generate agent-neutral runtime and MCP handoff files.")
    setup.add_argument("--agent", choices=KNOWN_AGENTS, default="generic")
    setup.add_argument("--workspace", default=None)
    setup.set_defaults(func=cmd_setup)

    mcp = sub.add_parser("mcp-config", help="Print a portable MCP server definition with absolute local paths.")
    mcp.add_argument("--workspace", default=None)
    mcp.set_defaults(func=cmd_mcp)

    instructions = sub.add_parser("instructions", help="Print canonical instruction and adapter paths.")
    instructions.add_argument("--agent", choices=KNOWN_AGENTS, default="generic")
    instructions.set_defaults(func=cmd_instructions)

    test = sub.add_parser("self-test", help="Verify the repository's agent-agnostic bootstrap files.")
